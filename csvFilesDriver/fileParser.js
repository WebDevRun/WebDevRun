export default class FileParser {
  ...
  #requisitesComparison = {
    ...
    detailedAnswer: ['задания с развёрнутым ответом', 'часть с развёрнутым ответом'],
    oralScore: ['устная часть'],
    prymaryScoreWrittenPart: ['первичный балл письменной части', 'первичный балл письм. части'],
  ...
  #normalize (string) {
    ...
    if (normalizeString.startsWith('"')) normalizeString = normalizeString.slice(1)
    if (normalizeString.endsWith('"')) normalizeString = normalizeString.slice(-1)
  ...
  #parseTitle (title) {
    ...
    if (splitTitle.includes(this.#english)) {
      const date = splitTitle[2].startsWith('п')
        ? `п:${new Date(splitTitle[2]).toLocaleDateString()}; у:${new Date(splitTitle[3]).toLocaleDateString()}`
        : `п:${new Date(splitTitle[3]).toLocaleDateString()}; у:${new Date(splitTitle[2]).toLocaleDateString()}`
  ...
  #serialize (parsedTableHead, parsedTableBody) {
    ...
    const head = parsedTableHead.filter((value, index) => {
      ...
      const isShortAnswer = this.#requisitesComparison.shortAnswer.includes(value)
      if (isShortAnswer) shortAnswerIndex = index
      return true
    })

    const body = parsedTableBody.map((array) => {
      array[shortAnswerIndex] = this.#convertShortAnswer(array[shortAnswerIndex])
  ...
  parseFile () {
    ...
    const parsedTableBody = this.#getTableBoby(tableBody, parsedTableHead.length)
